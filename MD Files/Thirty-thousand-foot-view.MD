...
│   ├── incomeSources: [             # EMBEDDED array (not subcollection)
│   │   { id, name, amount, frequency }
│   │ ]
│   └── allocations: {               # EMBEDDED map (not subcollection)
...
└── transactions/{transactionId}/
    ├── envelopeId: string
    ├── amount: number               # ALWAYS number, never string
    ├── date: Timestamp              # ALWAYS Timestamp, never string
    ├── month: string                # Denormalized for efficient queries
...
├─────────────────────────────────────────────────────────────┤
│                                                              │
│  User Action → Optimistic Update → Queue Sync → Firebase    │
│       │              │                  │            │       │
│       │              ▼                  │            │       │
│       │     Local State Updated         │            │       │
│       │     (immediate feedback)        │            │       │
│       │              │                  │            │       │
│       │              ▼                  ▼            │       │
│       │     Zustand Persist ←── Sync Queue ──→ Firestore    │
│       │     (IndexedDB)         (pending ops)               │
│       │                                                      │
└─────────────────────────────────────────────────────────────┘
```
...
┌─────────────────────────────────────────────────────────────┐
│                         VIEWS                                │
│  EnvelopeListView.tsx (1,148 lines) ◄── GOD COMPONENT       │
│  SettingsView.tsx (26,576 bytes)                            │
│  EnvelopeDetail.tsx, AddTransactionView.tsx, etc.           │
└─────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────┐
│                    ZUSTAND STORES                            │
│  envelopeStore.ts ──┬── envelopeStoreSync.ts (832 lines)    │
│                     ├── envelopeStoreEnvelopes.ts (539 lines)│
│                     ├── envelopeStoreTransactions.ts        │
│                     ├── envelopeStoreTemplates.ts           │
│                     ├── envelopeStoreSettings.ts            │
│                     ├── envelopeStoreRealtime.ts            │
│                     └── envelopeStoreNetwork.ts             │
│                                                              │
│  monthlyBudgetStore.ts (1,044 lines) ◄── SECOND GOD OBJECT  │
│  monthlyBudgetStoreRealtime.ts                              │
│  authStore.ts                                                │
└─────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────┐
│                       SERVICES                               │
│  EnvelopeService.ts, TransactionService.ts                  │
│  MonthlyBudgetService.ts, AppSettingsService.ts             │
...
┌─────────────────────────────────────────────────────────────┐
│                       FIREBASE                               │
│  users/{userId}/envelopes, transactions, incomeSources,     │
│  envelopeAllocations, monthlyBudgets, appSettings           │
└─────────────────────────────────────────────────────────────┘
```
...
export const EnvelopeListView: React.FC = () => {
  // 🚨 PROBLEM: Pulling from TWO stores with massive destructuring
  const { 
  ...
  // 🚨 PROBLEM: Business logic in the view layer
  ...
  // 🚨 PROBLEM: Complex filtering/sorting in component
...
const syncBudgetAllocationTransaction = async (...) => {
  ...
  // 🚨 PROBLEM: Cross-store mutation from inside a store
  // 🚨 PROBLEM: 90 lines of transaction sync logic in budget store
  // 🚨 PROBLEM: Date parsing scattered throughout
...
const importData = async (data: any) => {
  // 🚨 PROBLEM: 200+ lines for a single function
  // 🚨 PROBLEM: Mixes import, sync, and cleanup logic
  // 🚨 PROBLEM: Manual Firebase operations instead of using services
...
export interface FirestoreTransaction {
  amount: string;  // 🚨 String in Firebase
}

// models/types.ts
export interface Transaction {
  amount: number;  // 🚨 Number in app
}
...
### Phase 1: Low-Hanging Fruit (1-2 weeks) ✅ **COMPLETE**

**Goal:** Reduce complexity without structural changes.

#### 1.1 Extract Balance Calculation to Single Location ✅ **Complete**

**Status:** Moved to `src/hooks/useEnvelopeList.ts` - Balance calculation logic extracted from EnvelopeListView and centralized in a custom hook with proper memoization.

```typescript
// IMPLEMENTED: src/hooks/useEnvelopeList.ts
export const useEnvelopeList = () => {
  // Balance calculations now centralized and memoized
...
#### 1.2 Create Date Utility Module ✅ **Complete**

**Status:** `src/utils/dateUtils.ts` is now the source of truth for all date operations throughout the application.

```typescript
// IMPLEMENTED: src/utils/dateUtils.ts
...
**Refactored Files:**
- `monthlyBudgetStore.ts` - Replaced all manual date constructions
- `envelopeStoreSync.ts` - Standardized timestamp conversions  
- `envelopeStoreTransactions.ts` - Centralized month key generation
- `MonthlyBudgetService.ts` - Unified date handling

#### 1.3 Create Type Conversion Mappers ✅ **Complete**

**Status:** `src/mappers/transaction.ts` now enforces strict type safety with standardized `fromFirestore` and `toFirestore` functions.

```typescript
// IMPLEMENTED: src/mappers/transaction.ts
...
export const toFirestore = (tx: Partial<Transaction>, userId?: string): Record<string, any> => {
  // Ensures amount stored as string for Firebase compatibility
...
**Standardized Across:**
- `TransactionService.ts` - Updated imports and usage
- `envelopeStoreRealtime.ts` - Using new mapper functions
- `envelopeStoreTransactions.ts` - Replaced manual conversions
- `envelopeStoreSync.ts` - Centralized all transaction data handling
- `envelopeStoreEnvelopes.ts` - Eliminated inline parsing
- `envelopeStoreConverters.ts` - Simplified to use mappers

#### 1.4 Extract EnvelopeListView Logic to Custom Hook ✅ **Complete**

**Status:** `src/hooks/useEnvelopeList.ts` created with all business logic extracted from the 1,148-line component.

```typescript
// IMPLEMENTED: src/hooks/useEnvelopeList.ts
export const useEnvelopeList = () => {
  // All filtering, sorting, and derived state logic
  const visibleEnvelopes = useMemo(() => 
    envelopes.filter(env => !env.isPiggybank && env.isActive !== false)
  ...
  return {
    ...
    piggybanks, 
    availableToBudget,
    // ... all other derived state and handlers
  };
};
```

**Result:** EnvelopeListView component significantly reduced in complexity with all business logic properly separated into the MVVM pattern.
...
### 2026-01-21: Phase 2 & 3 Complete ✅ - Architecture Unified & Optimized

**Summary:** Successfully completed Phase 2 (Architecture Consolidation) and Phase 3 (Database Optimization). The application now operates on a unified `BudgetStore` with a clean, optimized data layer that adheres to the ideal "Greenfield" architecture.

**Accomplishments:**
- **Store Consolidation (Phase 2.1):** Merged `envelopeStore` and `monthlyBudgetStore` into a single `BudgetStore`, eliminating circular dependencies and simplifying state management.
- **Offline Architecture (Phase 2.3):** Dismantled the custom "Nervous System" sync queue. The app now relies fully on Firebase's native `enableIndexedDbPersistence` for robust offline support with optimistic updates.
- **Embedded Allocations (Phase 3.1):** `BudgetService` now reads and writes income sources and envelope allocations as **embedded maps/arrays** within `monthlyBudgets/{month}` documents. This has drastically reduced read operations.
- **Type Normalization (Phase 3.2):** Implemented strict mappers that force all transaction amounts to be stored as `numbers` in Firestore, removing the need for fragile string parsing in the UI.
- **Query Optimization (Phase 3.3):** Added a denormalized `month` field to all transactions, enabling efficient monthly querying without complex date filtering.

**Remaining Polish:**
- **Repository Pattern:** `BudgetService` is centralized but effectively acts as a hybrid Service/Repository. Strict separation is a low-priority polish item.
- **UI/UX:** Continuous refinement of animations (Card Stack, Moveable lists).

**Impact:**
- **Performance:** Significant reduction in Firestore reads due to embedded allocations.
- **Maintainability:** Single source of truth for state (`BudgetStore`) and data access (`BudgetService`).
- **Stability:** Offline mode is now handled by the platform SDK, reducing custom code surface area.

**Files Modified:**
- `src/stores/budgetStore.ts` (Unified store)
- `src/services/budgetService.ts` (Unified service with embedded logic)
- `src/mappers/transaction.ts` (Strict type enforcement)
- `src/hooks/useEnvelopeList.ts` (Logic extracted)

**Project Status:** ✅ **Architecture Complete**. Ready for Feature Development.

---

### 2026-01-16: Phase 1 Complete ✅ + Critical Bug Fixes

**Summary:** Successfully completed Phase 1 of the architecture refactor with significant improvements to code organization, type safety, and maintainability. Additionally resolved critical production bugs affecting piggybank functionality and budget calculations.

**Accomplishments:**
- **Refactored EnvelopeListView** to use a custom hook (`src/hooks/useEnvelopeList.ts`), implementing proper MVVM pattern separation
- **Standardized all date handling** to use `src/utils/dateUtils.ts` as the single source of truth across 6+ files
- **Implemented strict type mappers** for Transactions in `src/mappers/transaction.ts` with comprehensive error handling
- **Eliminated manual data conversions** throughout the codebase, replacing scattered `parseFloat()` and `toString()` calls
- **Updated all imports** to use new standardized function names (`fromFirestore`, `toFirestore`)
- **Resolved all build errors** and maintained backward compatibility

**Critical Bug Fixes Completed:**
- **Fixed "Invalid Creation Date" Crash:** Implemented robust `safeDateConversion` helper to handle Firestore Timestamp objects, raw objects, strings, and numeric dates
- **Fixed Missing Monthly Allocation Transactions:** Enhanced `setEnvelopeAllocation` and `updateEnvelopeAllocation` to automatically create/update/delete monthly transactions for piggybanks
- **Fixed Transaction Display Issues:** Corrected date format mismatch (ISO format with 'T' separator) preventing transactions from appearing in history
- **Fixed Piggybank Details View:** Updated filtering logic to always show automatic monthly allocation transactions regardless of creation date
- **Fixed Available to Budget Calculation:** Resolved $700 discrepancy by aligning calculation logic between hook and UI components

**Impact:**
- Reduced complexity in God components
- Improved type safety and eliminated runtime conversion errors  
- Centralized business logic in appropriate layers
- Established consistent patterns for data transformation
- Maintained full functionality while improving code structure
- **Piggybank system now fully functional** with automatic monthly contributions
- **Budget calculations now accurate** across all UI components

**Files Modified:** 15+ files including stores, services, mappers, utilities, and UI components
**Build Status:** ✅ All TypeScript compilation errors resolved
**Production Status:** ✅ All critical bugs resolved, piggybank system operational
