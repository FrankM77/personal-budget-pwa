...
const CardStackDemo: React.FC = () => {
  const [selectedCard, setSelectedCard] = useState<PaymentSource>(DATA[0]);
  const [isStackOpen, setIsStackOpen] = useState(false);
  const [isAddCardModalOpen, setIsAddCardModalOpen] = useState(false);
  const [cards, setCards] = useState<PaymentSource[]>(DATA);
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-black p-8">
      <div className="max-w-md mx-auto">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">Apple Wallet-Style Card Stack</h1>
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-gray-100 dark:border-zinc-800 shadow-sm p-6 space-y-6">
          ...
          <div>
            ...
            <div 
              onClick={toggleStack}
              className="flex items-center gap-4 p-3 border border-gray-300 dark:border-gray-600 rounded-lg cursor-pointer hover:bg-gray-50 dark:hover:bg-zinc-800 transition-colors"
            >
              <div 
                ...
                <AnimatePresence initial={false} mode="popLayout">
                  {[...cards, { id: 'add-new', name: '+ Add New Card', network: 'Visa' as const, last4: '', color: '#6b7280' }].map((card, index) => {
                    ...
                    return (
                      ...
                      >
                        <SwipeableRow 
                          onDelete={() => !isAddNew && handleDeleteCard(card)}
                          threshold={-80}
                        >
                          {/* Card */}
                          <div
